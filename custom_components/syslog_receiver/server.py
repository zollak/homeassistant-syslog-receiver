...
class SyslogServer:
    ...
    async def start(self):
        ...
        if protocol == "UDP":
            ...
            _LOGGER.debug("Started TCP on %s:%s", host, port)
    ...
    def process_message(self, data: bytes, addr):
        ...
        ips = self.options.get("allowed_ips", self.config.get("allowed_ips", []))
        ...
        if m:
            pri = int(m.group(1))
            severity = pri & 0x07
            min_level = MIN_SEVERITY_LEVELS.get(
                self.options.get("min_severity", self.config.get("min_severity", "info")), 6
            )
